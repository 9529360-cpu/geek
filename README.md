...
>
> 当前维护基线（2026-08-19）：客户端与 `package.json` 版本为 **1.2.12**，`.github/release-client-version` 为 **1.2.12**，Electron 锁定为 **43.4.0**。`npm test` 会动态发现并执行 `test/*.cjs` contract（仅排除两个 CDP 开发工具）；当前数量以 CI 输出为准，避免文档硬编码再次漂移。
