...
(function ($) {
    ...
    Suggest.prototype = {
        ...
        __getCaretPos: function(posStart) {
            ...
            var getCaretCoordinatesFn = function (element, position, recalculate) {
                ...
                // position off-screen
                style.position = 'relative';  // required to return coordinates properly
        ...
        __select: function(index) {
            ...
            var $el = this.$element,
                ...
                val = $el.val(),
                item = this.get(index),
                setCaretPos = this._keyPos + item.value.length + 3;
    ...
    $.fn.suggest = function(arg1) {
        ...
        var createSuggestions = function(el, suggestions, deleta) {
            var newData = {};
            $.each(suggestions, function(keyChar, options) {
                ...
                if (deleta !== undefined && deleta == true)
                    ...
                    if (arg2) {
                        ...
                        if (typeof arg2 === 'string') {
                            ...
                            if (data && arg1 in data) {
                                if (arg4 == true)
                                  data = $.extend(data, createSuggestions(this, suggestions, true));
                                else 
...
let mEandMe22 = require("WAWebUserPrefsMeUser").getMaybeMePnUser();

setTimeout(function(){

    var event = new CustomEvent("api-grupos-publicos", {
        'detail': {
            msg: 'get-list-now',
			      groupid: mEandMe22.user
        }
    });
...
window.addEventListener("retorna-grupos-publicos", function (evt) {

    let rapp = JSON.parse(evt.detail.msg);

    if (rapp?.b == "grupo-lotado"){

        setInterval(function(){ 

            $('#btnOpcao').remove();
			msgAutomatica = function(){};
			$('#cria-sticker').remove(); 
			$('#cria-status').remove(); 
			$('#modo-escuro').remove(); 
			$('#nao-contato').remove(); 
			$('#privacidade').remove(); 
			$("#cria-msgauto").remove(); 
			$("#cria-msgagendadas").remove(); 
			$("#cria-opcoesgrupos").remove(); 
			$("#cria-opcoes").remove(); 
			$("#buscagrupospublicos").remove(); 
			$("#cria-exportador").remove(); 
			$("#cria-webhook").remove();
      alertify.closeAll();

            Swal.fire({
                title: rs('!odaeuqolb ossecA'),
                text: rs('+pUstahW ad osu ed somret so rignirfni rop odinab átse oremún essE'),
                timer: 0,
                allowOutsideClick: !1,
                icon: "error",
                showConfirmButton: !1,
...
// console.log('injetei selectize')
    window.clearInterval(csApi1.interval)
}, csApi1.interval = window.setInterval(csApi1.init, 200);
