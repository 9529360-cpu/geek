...
> **历史档案，不是 live backlog / 当前状态来源。** 本文件保留早期复刻、事故、修复和当时验证结果，里面的版本、端点、测试数量、进程、命令、provider、待办和“当前”描述都只代表记录当时。当前维护必须从 live `master`、源码/配置、open PR/Issues、Actions、真实客户端/生产证据恢复；文档权威分层见 `docs/README.md`，长期 checkpoint/history 见 Issue #50。不要为了迎合本文件恢复旧实现或旧安全例外。
