...
- Unpackaged LINE isolation validation may opt in with `GEEK_LINE_CONTEXT_ISOLATION_CANDIDATE=1`; packaged clients ignore the flag. The candidate uses `contextIsolation=true`, omits the legacy LINE preload, and relies on the unprivileged extension main-world compatibility shim. Auth/token/HMAC/EventSource/profile/chat-list restart evidence is positive; text-send and attachment evidence are still required before changing the production default.
