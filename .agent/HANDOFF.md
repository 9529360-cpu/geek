...
- Unpackaged development userData is also workspace-owned. Every development profile stores only a SHA-256 workspace fingerprint in .geek-development-workspace.json; the same workspace may reuse it, but a different checkout using the same userData must fail closed. Alternate checkouts/tests must use their own GEEK_USER_DATA_DIR, which is independently workspace-bound.
...
- Unpackaged LINE isolation validation may opt in with `GEEK_LINE_CONTEXT_ISOLATION_CANDIDATE=1`; packaged clients ignore the flag. The candidate uses `contextIsolation=true` plus an integrity-covered isolated preload that exposes only `GeekLineDownloads.saveBlob`, while the LINE page keeps an unprivileged main-world compatibility shim. Auth/token/HMAC/EventSource/profile/chat-list restart evidence is positive. Synthetic same-origin Blob saves have completed the real chunked IPC -> Downloads path from small files through the embedded LINE 3.5.1 1 GiB limit, including bounded memory/latency measurement and cleanup; arbitrary external URLs are rejected. Authenticated LINE text-send, a real LINE chat attachment retrieval/decryption + save, and multi-account authenticated isolation evidence remain required before changing the production default.
...
- Account deletion's durable finalizer is the authority for global post-commit child cleanup: scheduled attachment refs and explicit legacy global `config.broadcastGroups` owned by the deleted account are replayed through the same idempotent cleanup debt. Startup also prunes explicit legacy group owners absent from authoritative Account State while preserving unowned compatibility records. Do not add a process-only deletion callback or edit `config.json` outside Config State.
