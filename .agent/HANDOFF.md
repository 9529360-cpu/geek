...
- Unpackaged LINE isolation validation may opt in with `GEEK_LINE_CONTEXT_ISOLATION_CANDIDATE=1`; packaged clients ignore the flag. The candidate uses `contextIsolation=true` plus an integrity-covered isolated preload that exposes only `GeekLineDownloads.saveBlob`, while the LINE page keeps an unprivileged main-world compatibility shim. Auth/token/HMAC/EventSource/profile/chat-list restart evidence is positive. A synthetic same-origin Blob has also completed the real chunked IPC -> Downloads path with byte-for-byte verification and immediate cleanup; arbitrary external URLs are rejected. The isolated download cap matches the embedded LINE 3.5.1 file limit of 1 GiB; near-limit memory/latency stress remains a production gate. Authenticated LINE text-send and a real LINE attachment download remain required before changing the production default.
