...
- Translation is a distributed value/result operation, not a blind HTTP retry. One logical translation must reuse one `X-Request-ID` across unknown transport outcomes and endpoint failover; D1 request identity/owner state remains authoritative. Same-ID `request_in_progress` is a nonterminal reconciliation state, completed same-semantics operations recover the already-committed result without another provider call or quota mutation, and semantic mismatch fails closed. Billing finalization and privacy-bounded encrypted replay metadata share one terminal commit; plaintext chat source/translation must not be persisted in D1. Translation reservation/replay schema migrations are production-data changes and must follow their explicit migration/deployment gate rather than being auto-run by Worker deployment.
