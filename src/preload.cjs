...
contextBridge.exposeInMainWorld(
  ...
  Object.freeze({
    ...
    quickScripts: Object.freeze({
      ...
      fill: (id) => ipcRenderer.invoke('quick-scripts:fill', id),
      requestEditor: (id) => ipcRenderer.invoke('quick-scripts:edit', id),
      importFile: () => ipcRenderer.invoke('quick-scripts:import'),
      exportFile: () => ipcRenderer.invoke('quick-scripts:export'),
      onChanged: (callback) => { ipcRenderer.on('quick-scripts:changed', () => callback()); },
      onFillRequest: (callback) => { ipcRenderer.on('quick-scripts:fill-request', (_event, id) => callback(id)); },
      onEditorRequest: (callback) => { ipcRenderer.on('quick-scripts:edit-request', (_event, id) => callback(id)); },
    }),
    quickPet: Object.freeze({
      toggle: () => ipcRenderer.invoke('quick-pet:toggle'),
      shape: (shape) => ipcRenderer.invoke('quick-pet:shape', shape),
      dragStart: (mouse) => ipcRenderer.invoke('quick-pet:drag-start', mouse),
      dragMove: (mouse) => ipcRenderer.invoke('quick-pet:drag-move', mouse),
      dragEnd: () => ipcRenderer.invoke('quick-pet:drag-end'),
    }),
