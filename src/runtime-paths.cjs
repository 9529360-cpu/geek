...
async function migrateRuntimeFiles({ userDataDir, projectRoot }) {
  ...
  const pairs = [
    { dest: accountsFile(userDataDir), legacy: legacyAccountsFile(projectRoot), allowEmptyShadow: true },
    { dest: configFile(userDataDir), legacy: legacyConfigFile(projectRoot), allowEmptyShadow: false }
  ];
  const results = [];
  for (const { dest, legacy, allowEmptyShadow } of pairs) {
    ...
    const destShadowEmpty = destExists && allowEmptyShadow
      ? isEmptyAccounts(await fs.readFile(dest, 'utf8').catch(() => ''))
      : false;
    if (destExists && !destShadowEmpty) {
