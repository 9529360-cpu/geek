...
function policyError(code, publicMessage) {
...
function requiredId(value, code) {
  ...
  if (!text) throw policyError(code);
  return text;
}

function normalizeStat(stat) {
  if (!stat || typeof stat.isFile !== 'function' || !stat.isFile()) {
    throw policyError('SCHEDULED_BROADCAST_ATTACHMENT_UNAVAILABLE', '定时附件已不可用，请重新选择。');
  }
...
function guessMime(filePath, pathModule) {
  return MIME_BY_EXT[pathModule.extname(filePath).toLowerCase()] || 'application/octet-stream';
}
...
function createScheduledBroadcastAttachmentStore(options = {}) {
  const fs = options.fs;
  if (!fs || typeof fs.realpath !== 'function' || typeof fs.stat !== 'function' || typeof fs.readFile !== 'function' || typeof fs.writeFile !== 'function' || typeof fs.rename !== 'function' || typeof fs.mkdir !== 'function') {
    throw new TypeError('fs with realpath/stat/readFile/writeFile/rename/mkdir is required');
  }
  ...
  async function registerPaths({ accountId, taskId, filePaths }) {
    ...
    for (const value of filePaths) {
      ...
      let stat;
      try {
        canonicalPath = await fs.realpath(String(value || ''));
      ...
      staged.push({
        canonicalPath,
        name: pathModule.basename(String(value || '')) || pathModule.basename(canonicalPath),
        size: current.size,
        mtimeMs: current.mtimeMs,
        mime: guessMime(canonicalPath, pathModule),
      });
    }
  ...
  async function resolve(refValue, { accountId, taskId } = {}) {
    ...
    let stat;
    try { stat = await fs.stat(entry.canonicalPath); }
    ...
    if (current.size !== entry.size || current.mtimeMs !== entry.mtimeMs) {
      throw policyError('SCHEDULED_BROADCAST_ATTACHMENT_CHANGED', '定时附件内容已发生变化，请重新选择。');
    }
    return Object.freeze({
  ...
  async function resolveMany(refValues, owner) {
    ...
    if (!refs.length || refs.length > limits.maxFiles || new Set(refs).size !== refs.length) {
      throw policyError('SCHEDULED_BROADCAST_ATTACHMENT_REF_INVALID');
    }
    const resolved = [];
    let totalBytes = 0;
    for (const ref of refs) {
      const item = await resolve(ref, owner);
      totalBytes += item.size;
      if (totalBytes > limits.maxTotalBytes) throw policyError('BROADCAST_FILE_TOTAL_LIMIT', '附件总大小不能超过 1 GiB。');
      resolved.push(item);
    }
  ...
  function listTask(accountId, taskId) {
    ...
    return Object.freeze([...entries.values()]
      .filter(entry => entry.accountId === owner && entry.taskId === task)
      .map(publicRecord));
  }
...
module.exports = {
  DEFAULT_LIMITS,
  createScheduledBroadcastAttachmentStore,
};
