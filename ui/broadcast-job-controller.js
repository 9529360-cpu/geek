(function (root, factory) {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .bc-job-head{display:flex;align-items:center;gap:8px}.bc-job-dot{width:7px;height:7px;border-radius:50%;background:var(--accent);box-shadow:0 0 0 3px var(--accent-soft)}
      ...
      #broadcast-overlay .bc-action-trigger .bc-switch,#broadcast-overlay .bc-action-trigger input{display:none!important}
      #broadcast-overlay .bc-action-trigger .bc-switch-label{font-size:11px;font-weight:600}
    `;
  ...
  function ensureBar(runtime) {
    ...
    bar.innerHTML = `
      ...
      <div class="bc-job-actions"><button type="button" data-act="pause">暂停</button><button type="button" class="danger" data-act="stop">停止</button><button type="button" data-act="failures" class="hidden">查看失败</button></div>`;
    document.body.appendChild(bar);
    bar.querySelector('[data-act="pause"]').onclick = () => document.getElementById('broadcast-pause')?.click();
    bar.querySelector('[data-act="stop"]').onclick = () => document.getElementById('broadcast-stop')?.click();
    bar.querySelector('[data-act="failures"]').onclick = () => {
      const box = bar.querySelector('.bc-job-failures');
      const details = Array.isArray(window.__lastFailDetail) ? window.__lastFailDetail : [];
      box.textContent = details.length ? details.join('\n') : '失败明细暂不可用，可使用失败名单导出。';
      box.classList.toggle('hidden');
    };
    bar.querySelector('.bc-job-close').onclick = () => {
      if (!runtime.job || !TERMINAL.has(runtime.job.state)) return;
      runtime.job.dismissed = true;
      render(runtime);
    };
    return bar;
  }
  ...
  function render(runtime) {
    ...
    if (job.state === 'completed') {
      ...
      meta.textContent = job.nextSeconds != null ? `成功 ${job.ok} · 失败 ${job.fail} · 下一条 ${job.nextSeconds} 秒后` : `成功 ${job.ok} · 失败 ${job.fail}`;
    }
    const terminal = TERMINAL.has(job.state);
    pause.classList.toggle('hidden', terminal);
  ...
  function install() {
    ...
    const beginObservation = () => {
      ...
      runtime.monitor = setInterval(() => {
        ...
        if (progress) { runtime.job.current = progress.current; runtime.job.total = progress.total; }
        runtime.job.ok = Number(document.getElementById('bc-sent-count')?.textContent) || runtime.job.ok || 0;
        const countdown = document.getElementById('bc-countdown')?.textContent || '';
        const seconds = countdown.match(/00:(\d+)s/);
        runtime.job.nextSeconds = seconds ? Number(seconds[1]) : null;
        const pauseText = document.getElementById('broadcast-pause')?.textContent || '';
        if (!TERMINAL.has(runtime.job.state)) runtime.job.state = pauseText.includes('继续') ? 'paused' : 'running';
        if (completion) {
          runtime.job.ok = completion.ok;
          runtime.job.fail = completion.fail;
    ...
    document.addEventListener('click', (event) => {
      ...
      if (!target) return;
      if (runtime.job && !TERMINAL.has(runtime.job.state)) {
      ...
      setTimeout(() => {
        ...
        const started = sending && !sending.classList.contains('hidden') || /发送中|已暂停/.test(progress);
        if (!started || !runtime.pendingOwner) return;
        runtime.job = createJob(runtime.pendingOwner, { state: 'running', total: Number(document.getElementById('bc-total-count')?.textContent) || selectedCount() });
