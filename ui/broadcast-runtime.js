(function (root, factory) {
  ...
  function selectedChatTargets() {
    return [...document.querySelectorAll('#bc-selected-chips .bc-selected-chip')]
      .map(chip => {
        const button = chip.querySelector('button[data-id]');
        const id = String(button?.dataset?.id || '').trim();
        if (!id) return null;
        const name = String(chip.querySelector('span')?.textContent || id).trim() || id;
        return { id, name };
      })
      .filter(Boolean);
  }
  ...
  function selectedChatIds() {
    return selectedChatTargets().map(target => target.id);
  }
  ...
  return Object.freeze({ install, activeAccountId, personalize, dedupeTargets, validateContent, shouldFailContextInitialization, liveGuestId, selectedChatTargets });
});
