(function (root, factory) {
  ...
  function jobStatusLabel(job) {
    if (!job) return '';
    const labels = {
      starting: '准备中', running: '发送中', paused: '已暂停', stopping: '停止中',
      scheduled: '已定时', queued: '排队中', completed: '已完成', stopped: '已停止', failed: '有异常',
    };
    return labels[job.state] || String(job.state || '');
  }
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .bc-workbench-step{position:relative;display:flex;align-items:center;gap:7px;min-width:0;padding:8px 9px;border:1px solid transparent;border-radius:9px;background:transparent;color:var(--text-tertiary);cursor:pointer;text-align:left;transition:background .18s ease,border-color .18s ease,color .18s ease,transform .18s ease}
      ...
      #broadcast-overlay .bc-body{padding:12px;gap:10px;background:color-mix(in srgb,var(--bg-elevated) 94%,var(--bg-surface));max-height:calc(100vh - 184px)}
      #broadcast-overlay .bc-card{border-radius:10px;padding:12px;border-color:var(--border-standard);box-shadow:0 1px 0 rgba(255,255,255,.02)}
      #broadcast-overlay .bc-card[data-workbench-hidden="true"]{display:none!important}
      #broadcast-overlay .bc-card-title{font-size:13px;margin-bottom:12px}
      .bc-review-card{display:flex;flex-direction:column;gap:10px}
      .bc-review-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}
      .bc-review-item{padding:10px;border:1px solid var(--border-standard);border-radius:9px;background:var(--bg-elevated)}
      .bc-review-item small{display:block;color:var(--text-tertiary);font-size:10.5px;margin-bottom:4px}.bc-review-item strong{font-size:12px;word-break:break-word}
      .bc-review-preview{padding:10px;border-radius:9px;background:var(--bg-elevated);border:1px solid var(--border-standard)}
      .bc-review-preview small{display:block;color:var(--text-tertiary);margin-bottom:5px}.bc-review-preview p{margin:0;white-space:pre-wrap;max-height:110px;overflow:auto;font-size:11.5px;line-height:1.55}
      ...
      #broadcast-overlay .bc-footer__btns{align-items:center;gap:8px}.bc-workbench-next{min-width:108px}
      ...
      .bc-workbench-status i{width:8px;height:8px;border-radius:50%;background:var(--accent);box-shadow:0 0 0 4px var(--accent-soft)}
      .bc-workbench-status[data-state="loading"] i{animation:bc-workbench-pulse 1s ease-in-out infinite}
      @keyframes bc-workbench-pulse{0%,100%{opacity:.35;transform:scale(.82)}50%{opacity:1;transform:scale(1.15)}}
      .bc-workbench-toast{position:fixed;right:18px;bottom:20px;z-index:240;min-width:260px;max-width:390px;padding:11px 12px;border:1px solid var(--border-standard);border-radius:10px;background:color-mix(in srgb,var(--bg-surface) 96%,transparent);box-shadow:0 18px 50px rgba(0,0,0,.34);backdrop-filter:blur(16px);color:var(--text-primary);font-size:11.5px;display:flex;align-items:center;gap:9px;animation:bc-toast-in .18s ease-out}
      .bc-workbench-toast button{margin-left:auto;border:0;border-radius:7px;padding:5px 8px;background:var(--accent-soft);color:var(--accent);cursor:pointer}
      ...
      .bc-task-center-overlay{position:fixed;inset:0;z-index:250;background:rgba(0,0,0,.38);display:grid;place-items:center;backdrop-filter:blur(3px)}.bc-task-center-overlay.hidden{display:none}
      .bc-task-center{width:min(650px,calc(100vw - 32px));max-height:min(680px,calc(100vh - 40px));overflow:hidden;border:1px solid var(--border-standard);border-radius:14px;background:var(--bg-surface);box-shadow:0 24px 80px rgba(0,0,0,.42);display:flex;flex-direction:column}
      .bc-task-center-head{display:flex;align-items:center;padding:13px 15px;border-bottom:1px solid var(--border-standard)}.bc-task-center-head strong{font-size:14px}.bc-task-center-head small{margin-left:8px;color:var(--text-tertiary)}.bc-task-center-head button{margin-left:auto;border:0;background:transparent;color:var(--text-tertiary);font-size:20px;cursor:pointer}
      .bc-task-center-body{overflow:auto;padding:12px;display:flex;flex-direction:column;gap:8px}.bc-task-row{padding:10px 11px;border:1px solid var(--border-standard);border-radius:9px;background:var(--bg-elevated)}.bc-task-row-head{display:flex;align-items:center;gap:8px}.bc-task-row-head strong{font-size:12px}.bc-task-state{padding:2px 6px;border-radius:999px;background:var(--accent-soft);color:var(--accent);font-size:9.5px}.bc-task-row-meta{margin-top:5px;color:var(--text-tertiary);font-size:10.5px}.bc-task-actions{display:flex;gap:6px;margin-top:8px}.bc-task-actions button{height:26px;padding:0 8px;border:1px solid var(--border-standard);border-radius:6px;background:var(--bg-surface);color:var(--text-secondary);font-size:10.5px;cursor:pointer}
      .bc-task-history-title{margin:7px 2px 0;color:var(--text-secondary);font-size:11px;font-weight:700}.bc-task-empty{padding:26px 12px;text-align:center;color:var(--text-tertiary);font-size:11.5px}
  ...
  function showToast(message, actionText, onAction, persist = false) {
    ...
    if (actionText && typeof onAction === 'function') {
      ...
      button.onclick = () => { onAction(); toast.remove(); };
  ...
  function webviewForAccount(account) {
    ...
    return [...document.querySelectorAll('webview')].find(wv => String(wv.partition || wv.getAttribute?.('partition') || '') === String(account.partition || '')) || null;
  }
  ...
  async function probeWhatsAppReadiness(account) {
    ...
    try {
      ...
      return { ready: false, reason: '正在加载群发组件' };
    }
  }
  ...
  async function awaitBroadcastReadiness(account, options = {}) {
    ...
    for (let attempt = 0; attempt < attempts; attempt++) {
      ...
      if (typeof options.onProgress === 'function') options.onProgress({ ...state, attempt, attempts });
  ...
  function ensureInlineStatus() {
    ...
    if (!status) {
      ...
      status.innerHTML = '<i></i><span>联系人与群组准备就绪后会自动显示</span>';
  ...
  function ensureReviewCard() {
    ...
    card.innerHTML = `
      <h3 class="bc-card-title"><span>检查发送</span><small>确认发送账号、对象、内容和时间</small></h3>
      <div class="bc-review-grid">
        <div class="bc-review-item"><small>发送账号</small><strong data-review="account">—</strong></div>
        <div class="bc-review-item"><small>发送对象</small><strong data-review="audience">—</strong></div>
        <div class="bc-review-item"><small>发送时间</small><strong data-review="time">—</strong></div>
        <div class="bc-review-item"><small>发送间隔</small><strong data-review="interval">—</strong></div>
        <div class="bc-review-item"><small>附件 / 名片</small><strong data-review="extras">—</strong></div>
        <div class="bc-review-item"><small>任务方式</small><strong data-review="mode">固定受众快照</strong></div>
      </div>
      <div class="bc-review-preview"><small>消息预览</small><p data-review="message">未填写文本消息</p></div>`;
  ...
  async function refreshReview() {
    ...
    const values = {
      account: account ? `${account.name || '当前账号'} · ${String(account.type || '').replace('whatsapp-pure', 'WhatsApp').replace('telegram-z', 'Telegram').replace('telegram-k', 'Telegram')}` : '当前账号',
  ...
  function setStep(step) {
    ...
    document.querySelectorAll('#broadcast-overlay .bc-card').forEach(card => {
      ...
      if (!own) return;
      card.dataset.workbenchHidden = own === step ? 'false' : 'true';
    });
  ...
  function ensureStepper() {
    ...
    const labels = [['content', '1', '消息内容'], ['audience', '2', '发送对象'], ['settings', '3', '发送设置'], ['review', '4', '检查发送']];
    for (const [id, num, label] of labels) {
      const button = document.createElement('button');
      button.type = 'button';
  ...
  function ensureFooterNavigation() {
    ...
    prev.id = 'broadcast-workbench-prev'; prev.type = 'button'; prev.textContent = '上一步';
    const next = document.createElement('button');
    next.id = 'broadcast-workbench-next'; next.type = 'button'; next.className = 'bc-workbench-next'; next.textContent = '下一步';
  ...
  function resetWorkbenchOnOpen() {
    ...
    setTimeout(() => {
      ...
      else if (/失败/.test(text)) setInlineStatus('联系人仍在同步，可稍后重试或重新打开群发', 'loading');
    }, 650);
  }
  ...
  function jobRow(job, manager) {
    ...
    if (!TERMINAL.has(job.state)) {
      ...
      if (job.state === 'running' || job.state === 'paused') {
        const pause = document.createElement('button'); pause.type = 'button'; pause.textContent = job.state === 'paused' ? '继续' : '暂停';
        pause.onclick = async () => { await manager.invoke(job.id, job.state === 'paused' ? 'resume' : 'pause').catch(() => {}); void renderTaskCenter(); };
        actions.appendChild(pause);
      }
      const stop = document.createElement('button'); stop.type = 'button'; stop.textContent = job.state === 'scheduled' ? '取消定时' : job.state === 'queued' ? '取消排队' : '停止后续发送';
      stop.onclick = async () => { await manager.invoke(job.id, 'stop').catch(() => {}); void renderTaskCenter(); };
      actions.appendChild(stop); row.appendChild(actions);
    }
    return row;
  }
  ...
  async function renderTaskCenter() {
    ...
    if (jobs.length) jobs.forEach(job => body.appendChild(jobRow(job, manager)));
      ...
      const empty = document.createElement('div'); empty.className = 'bc-task-empty'; empty.textContent = '当前账号还没有本次会话的群发任务'; body.appendChild(empty);
    }
    let history = [];
    try {
  ...
  function ensureTaskButton() {
    ...
    const button = document.createElement('button');
    button.id = 'broadcast-task-center-button'; button.type = 'button'; button.className = 'bc-task-center-button'; button.textContent = '任务'; button.title = '查看当前账号群发任务';
    button.onclick = event => { event.stopPropagation(); openTaskCenter(); };
    trigger.insertAdjacentElement('afterend', button);
  }

  async function interceptBroadcastOpen(event) {
    const open = event.target?.closest?.('#bc-menu-send');
    if (!open || reopenGuard) return;
    const accountId = activeAccountId();
    const accounts = await listAccounts().catch(() => []);
    const account = accounts.find(item => String(item.id) === String(accountId));
    if (!account || !String(account.type || '').startsWith('whatsapp')) return;
    event.preventDefault(); event.stopImmediatePropagation();
    showToast('正在准备 WhatsApp 群发联系人…', '', null, true);
    const readiness = await awaitBroadcastReadiness(account, {
      onProgress: state => showToast(state.reason || '正在准备 WhatsApp 群发联系人…', '', null, true),
    });
    document.getElementById('broadcast-workbench-toast')?.remove();
    if (!readiness.ready) {
      showToast('WhatsApp 群发组件仍在初始化，可直接重试。', '重试', () => open.click(), true);
      return;
    }
    reopenGuard = true;
    try { open.click(); } finally { queueMicrotask(() => { reopenGuard = false; resetWorkbenchOnOpen(); }); }
  }
  ...
  function install() {
    ...
    installed = true;
    injectStyles();
    ensureStepper(); ensureFooterNavigation(); ensureReviewCard(); ensureTaskButton(); ensureTaskCenter();
    setStep('content');

    document.addEventListener('click', event => { void interceptBroadcastOpen(event); }, true);
