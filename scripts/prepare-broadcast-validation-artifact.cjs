...
if (!fs.existsSync(source)) {
  throw new Error(`Missing validation installer: ${source}`);
}

const outDir = path.join(root, 'dist-validation');
fs.mkdirSync(outDir, { recursive: true });
const short = String(process.env.GITHUB_SHA || 'local').slice(0, 8);
const installerName = `geek-${pkg.version}-broadcast-final-candidate-${short}.exe`;
...
const manifest = {
  purpose: 'broadcast-final-candidate-real-client-validation',
...
fs.writeFileSync(
  path.join(outDir, 'validation-manifest.json'),
