...
console.log(`RUNTIME node=${process.versions.node} electron=${process.versions.electron || 'none'} chromium=${process.versions.chrome || 'none'}`);
...
const email = `electron43-translate-smoke-${suffix}@example.invalid`;
...
async function cleanup() {
  ...
  await d1Query('DELETE FROM translation_usage WHERE user_id = ?;', [userId]).catch(() => {});
  await d1Query('DELETE FROM users WHERE id = ?;', [userId]).catch(() => {});
  console.log('OK transient smoke cleanup');
}

try {
  const health = await jsonRequest(`${translateBase}/health`);
  requireOk('translate-health', health);
  ...
  const translated = await jsonRequest(`${translateBase}/v1/translate`, {
    ...
    body: JSON.stringify({
      text: 'Electron runtime translation smoke.',
      source: 'en',
      target: 'zh',
      provider: 'auto',
      route: 'default',
  ...
  if (typeof translated.payload?.text !== 'string' || !translated.payload.text.trim()) throw new Error('translate returned empty text');
  console.log(`OK engine=${String(translated.payload?.engine || 'unknown')}`);

  const rows = await d1Query('SELECT status, completed_at FROM translation_usage WHERE request_id = ?;', [requestId]);
  if (!rows[0] || rows[0].status !== 'complete' || !rows[0].completed_at) throw new Error('translation D1 receipt incomplete');
  console.log('ELECTRON_TRANSLATION_RUNTIME_SMOKE_OK');
} finally {
  await cleanup();
}
