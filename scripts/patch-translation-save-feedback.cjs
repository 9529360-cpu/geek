...
const legacyContractPath = path.join(root, 'test', 'translation-settings-ux-contract.cjs');
let legacyContract = fs.readFileSync(legacyContractPath, 'utf8');
legacyContract = replaceOnce(
  ...
  'legacy translation failure contract'
);
fs.writeFileSync(legacyContractPath, legacyContract);
