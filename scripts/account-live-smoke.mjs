...
try {
  ...
  if (!Number.isSafeInteger(Number(register.payload?.userId)) || Number(register.payload.userId) < 1) {
    throw new Error('register returned invalid userId');
  }
  registered = true;
  ...
  if (typeof login.payload?.token !== 'string' || login.payload.token.length < 40) {
    throw new Error('login returned invalid token');
  }
  if (Number(login.payload?.user?.id) !== registeredUserId || login.payload?.user?.email !== email) {
    throw new Error('login returned mismatched user identity');
  }
  const setCookie = login.response.headers.get('set-cookie') || '';
  for (const required of ['HttpOnly', 'Secure', 'SameSite=Strict']) {
    if (!setCookie.includes(required)) throw new Error(`login cookie missing ${required}`);
  }
  ...
  if (!Number.isFinite(Number(status.payload?.remaining_chars))) {
    throw new Error('status returned invalid remaining_chars');
  }

  const me = await jsonRequest(`${apiBase}/api/me`, { headers: authHeaders });
  requireOk('me', me);
  if (Number(me.payload?.user?.id) !== registeredUserId || me.payload?.user?.email !== email) {
    throw new Error('me returned mismatched user identity');
  }
