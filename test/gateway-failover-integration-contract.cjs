...
assert.match(runtime, /pool\.pick\(body\.route\)/, '翻译请求必须把规范化 route 交给唯一网关池 owner 选端点');
...
assert.match(runtime, /route:\s*picked\.route/, 'Worker 必须收到实际选中的 primary|backup 线路，而不是仅回显请求标签');
assert.match(runtime, /body\.route === 'primary'/, '显式主线路必须拥有独立的端点尝试边界');
assert.match(runtime, /body\.route === 'backup'/, '显式备用线路必须拥有独立的端点尝试边界');
