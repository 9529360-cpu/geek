...
assert.match(workflow, /function Invoke-PublicRequestWithRetry/, 'rollback preflight must use a bounded public-request helper');
...
assert.match(workflow, /Invoke-PublicRequestWithRetry "\$root\/\$previousPath\?before=\$nonce" 'Head'/, 'previous installer verification must use the declared path and retry helper');
assert.match(workflow, /Invoke-PublicRequestWithRetry "\$root\/\$previousPath\.blockmap\?before=\$nonce" 'Head'/, 'previous blockmap verification must use the declared path and retry helper');
...
assert.match(workflow, /public request failed after \$Attempts attempts/, 'retry exhaustion must produce an explicit failure');
