...
(async () => {
...

  {
    ...
    assert.equal(harness.pendingPartitionDeletions.has(harness.partDir), true, 'repeated filesystem removal failure must retain the exact exit-time retry path');
    assert.equal(harness.calls.filter(call => call === `rm:${harness.partDir}`).length, 5, 'filesystem removal must keep the existing bounded five-attempt retry contract');
    assert.equal(harness.calls.at(-1), 'notify', 'directory cleanup failure must still publish the committed account state');
  }

  {
    ...
    const result = await harness.removeAccount({}, 'acct-3');
    assert.deepEqual(result, { activeAccountId: null, accounts: [] });
