...
for (const id of ['contact-notes-name', 'contact-notes-country', 'contact-notes-source', 'contact-notes-follow-status', 'contact-notes-next-follow-up', 'contact-notes-text']) {
  assert.match(html, new RegExp(`id="${id}"`), `客户资料字段 ${id} 必须存在`);
}
...
const first = notes.updateStore('', tg, {
  name: ' 张总 ', country: '德国', source: '展会', followStatus: 'following',
  nextFollowUp: '2026-09-01T10:30', notes: '  重点客户  ',
}, 123);
assert.equal(first.items[tg.key].name, '张总');
assert.equal(first.items[tg.key].notes, '重点客户');
assert.equal(notes.profileFor(JSON.stringify(first), tg).followStatus, 'following');
const legacy = { version: 1, items: { [tg.key]: { text: '旧版备注' } } };
assert.equal(notes.profileFor(JSON.stringify(legacy), tg).notes, '旧版备注', '第一版纯文本备注必须无损兼容');
const removed = notes.updateStore(JSON.stringify(first), tg, {}, 124);
assert.equal(Object.keys(removed.items).length, 0, '保存空备注必须删除记录');
assert.throws(() => notes.updateStore('', tg, { notes: 'x'.repeat(2001) }), /2000/);
assert.throws(() => notes.updateStore('', tg, { nextFollowUp: 'tomorrow' }), /格式无效/);
