...
assert.equal(converted.legacy.length, 2);
assert.equal(converted.migrated.length, 1, 'legacy task with a persisted group set should migrate');
...
assert.equal(converted.needsReview.length, 1, 'legacy current-selection task must require review because recipients were never persisted');
assert.match(converted.needsReview[0].reason, /未持久化收件人/);
