...
assert.match(desktopIpc, /ipcMain\.handle\(channel, async \(event, \.\.\.args\) => \{\s*assertMainFrameIpcSender\(event\);\s*assertTrustedSender\(event\);\s*return handler\(\.\.\.args\);/s, 'Desktop IPC owner must bind the exact sending main frame before broader sender authorization and app version handling');
