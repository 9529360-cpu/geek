...
async function main() {
  ...
  assert.match(config, /wdio-before-session/);
  assert.match(config, /wdio-session-established/);
  ...
  assert.match(workflow, /\[Environment\]::UserInteractive/);
  assert.match(workflow, /SessionId/);
  ...
  assert.match(mainEntry, /const e2eShellEnabled = installSubscriptionStartupBypass/);
  assert.match(mainEntry, /\[geek-e2e\] startup/);
  assert.match(mainEntry, /window-created/);
